...
import { Calendar, ArrowRight, Loader2, Search, ChevronLeft, ChevronRight } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen relative" dir="rtl">
      ...
      <div className="relative z-10">
        ...
        {/* Hero Section */}
        <section className="relative bg-gradient-to-br from-orange-50 to-orange-100" style={{ paddingTop: '10rem', paddingBottom: '8rem' }}>
          {/* Artboard Background */}
          <div
          ...
          <div className="max-w-7xl mx-auto relative z-10" style={{ paddingLeft: '1rem', paddingRight: '1rem' }}>
            <div className="text-center">
              <h1 className="text-4xl md:text-5xl text-gray-900 leading-tight animate-fade-in-up" style={{ fontWeight: 500, marginBottom: '1.5rem' }}>
                פוסטי בלוג
              </h1>
              <p className="text-xl text-gray-700 max-w-2xl mx-auto animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
        ...
        {/* Main Content */}
        <main className="max-w-7xl mx-auto" style={{ paddingLeft: '1rem', paddingRight: '1rem', paddingTop: '4rem', paddingBottom: '4rem' }}>

          {/* Search Bar */}
          <div className="max-w-md mx-auto" style={{ marginBottom: '3rem' }}>
            <div className="relative">
              <div className="absolute inset-y-0 top-[50%] translate-y-[-50%] right-0 flex items-center" style={{ paddingRight: '0.75rem' }}>
          ...
          {!loading && (
            <div className="text-center text-white" style={{ marginBottom: '2rem' }}>
              {searchQuery ? (
                ...
                <p>
                  סה"כ {blogPosts.length} פוסטים בבלוג
                </p>
              )}
            </div>
          )}
          ...
          {filteredPosts.length === 0 && !loading ? (
            ...
            <div className="text-center" style={{ paddingTop: '4rem', paddingBottom: '4rem' }}>
              <div className="bg-white rounded-2xl shadow-lg max-w-md mx-auto" style={{ padding: '3rem' }}>
                ...
                <p className="text-gray-600">
                  {searchQuery ? 'נסה לחפש במילים אחרות' : 'הפוסטים הראשונים יפורסמו בקרוב'}
                </p>
              </div>
            </div>
          ) : (
            ...
            <div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {currentPosts.map((post) => (
                  <Card key={post.id} className="overflow-hidden hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 bg-white border-0 shadow-lg group h-full">
                    {/* Thumbnail */}
                    {post.thumbnail_url && (
                    ...
                    <CardContent className="space-y-4 flex flex-col h-full" style={{ padding: '1.5rem' }}>
                      {/* Date */}
                      ...
                      {post.summary && (
                       ...
                       {post.tags && post.tags.length > 0 && (
                         <div className="flex flex-wrap gap-2 border-t" style={{ paddingTop: '1rem' }}>
              ...
              {/* Pagination */}
              {totalPages > 1 && (
                <div className="flex justify-center items-center space-x-reverse gap-2" style={{ marginTop: '3rem' }}>
                  <Button
                    variant="outline"
              ...
              {/* Page Info */}
              {totalPages > 1 && (
                <div className="text-center text-sm text-gray-500" style={{ marginTop: '1rem' }}>
                  מציג {startIndex + 1}-{Math.min(endIndex, filteredPosts.length)} מתוך {filteredPosts.length} פוסטים
                </div>
              )}
            </div>
          )}
          ...
          {filteredPosts.length > 0 && (
            <div className="text-center bg-white rounded-2xl shadow-lg" style={{ marginTop: '4rem', padding: '2rem' }}>
              ...
              <p className="text-gray-600 max-w-2xl mx-auto" style={{ marginBottom: '1.5rem' }}>
                הפוסטים נותנים מידע כללי, אבל כל מקרה הוא ייחודי.
                בואו נדבר על המצב הספציפי שלכם
              </p>
              <Button
                size="lg"
